...
app = FastAPI(title="Surya Mishra")
...
def _get_poetry_dir() -> str:
    return os.path.join(os.path.dirname(__file__), "content", "poetry")
...
def list_poems() -> List[Dict]:
    """Scan the poetry folder and return metadata for each markdown poem."""
    posts = []
    posts_dir = _get_poetry_dir()
    pattern = os.path.join(posts_dir, "*.md")
...
@app.get("/poetry", response_class=HTMLResponse)
async def poetry(request: Request):
    posts = list_poems()
    return templates.TemplateResponse("poetry_list.html", {"request": request, "posts": posts})
...
async def career(request: Request):
    ...
    return templates.TemplateResponse("career.html", {"request": request, "projects": projects})
